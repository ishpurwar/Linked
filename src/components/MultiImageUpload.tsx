...
export default function MultiImageUpload({ 
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to upload one or more images';
      alert(errorMessage);
    } finally {
      setUploading(false);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {Array.from({ length: maxImages - images.length }).map((_, index) => (
          <div
            key={`empty-${index}`}
            className="w-full h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center text-gray-500 cursor-pointer hover:border-gray-400"
            onClick={() => document.getElementById('image-upload')?.click()}
          >
      ...
      <p className="text-xs text-gray-500">
        Tip: Upload multiple images to create a more attractive profile. Maximum {maxImages} images.
      </p>
    </div>
  );
}
